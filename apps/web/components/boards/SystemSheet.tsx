...
export function SystemSheet({ sheets }: { sheets: SystemSheetEntry[] }) {
  ...
  return (
    <div className="card overflow-hidden">
      <TierTabs
        items={sheets.map((s) => ({ id: s.tier, label: s.label, meta: `${s.parts} parts` }))}
        active={active.tier}
        onSelect={(t) => select(`#architecture-${t}`)}
        controls={panelId}
        status={
          `${active.facts.power.stages.length} power stages · ${buses.length} buses · ` +
          `${active.facts.power.draw} W` +
          (active.facts.power.headroom !== null
            ? ` · ${active.facts.power.headroom}% headroom`
            : '')
        }
      />
      ...
      <div className="flex flex-wrap items-baseline gap-x-5 gap-y-1 border-t border-[var(--line)] px-4 py-3 text-[12.5px] text-[var(--ink-2)]">
        <span>
          Every part in the tier appears here. A part that does not is a build failure, not a tidier
          diagram.
        </span>
