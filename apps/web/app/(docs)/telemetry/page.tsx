...
import { Container, Note, PageHeader } from '../../../components/ui'
  ...
  return (
    <>
      <PageHeader
        ...
        title="One chart per band, one clock underneath all of them."
        lede={
          <>
        ...
        {feed.kind === 'mock' && (
          <Note kind="info" title="This is a synthetic feed">
            <p>
              ...
              floor. The generator is deterministic in its inputs, so the same window always renders
              the same data; the window itself advances with the clock, which is what makes the
              charts move. Pin the scrub to a fixed offset and a reload reproduces the view exactly.
              Switching to a live grid is one environment variable and no code changes above the
              feed interface.
            </p>
          </Note>
        )}
